...
README.md、CURRENT_STATUS.md 的唯一编辑源为本目录；决定源为 `/Volumes/Leny/Projects/Zhanbu/Drawing/DECISION_EVENTS.md`。三个记录文件同步为 ProjectRecord 的同名逐字副本；只在唯一源编辑，消费者不得反向修改副本。
...
- PR 入口：`/Volumes/Leny/ProjectRecord/Drawing`。README 与状态的相对路径以唯一编辑源目录解析；PR 不复制源码、素材或 Agent/Skill 定义。
