...
- 2026-08-29 已更新未签名 `Divination/Divination.ipa`：Payload 为 iphoneos/arm64 `Divination.app`，Bundle ID 为 `local.divination.app`、最低 iOS 16.0，包含新图标与算法资源；确认无 `_CodeSignature`、embedded mobileprovision、AppleDouble 或 `__MACOSX`，SHA-256 为 `1e73b2ea2fd8ff93549b0ee7293e177b59693e34a8d0f351f985abcd50af22b9`。
...
1. 用户安装 `Divination.ipa` 后人工验收新图标、综合摘要、日期预测择日输入，以及 iOS 新增七种方法的第二排标签、长盘面、复制与历史记录；构建与 IPA 完整性检查不等于真机运行验收。
2. 在 Windows `.NET Framework csc.exe` 环境编译启动 WinForms，确认 exe 使用新图标且七个入口能从 `Divination.exe` 同目录打开离线 PWA；当前 Mac 无 Windows C# 编译器。
3. 发布 PWA 后确认 `divination-v15` 能替换旧缓存，并在断网状态下加载 1.6 MB 算法包、显示新图标并完成七种排盘。
