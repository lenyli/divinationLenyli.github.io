# Divination · Current Status

项目状态：已完成


2026-09-02 用户确认项目完成并关闭。

> 唯一当前状态；项目规则见 [README.md](README.md)。验证基线沿用原记录，本次文档整理未重跑。

## 项目与硬约束

- Divination 是 Zhanbu 目录下的独立项目；Drawing 另行维护自己的 README、状态与决定。
- Divination 以 `Divination.cs` 为权威数据来源。

## 当前完成状态

- Divination 的 PWA 已完成并部署，另有 Windows、iOS、macOS 版本。
...
- 单项结果不再显示算法版本、来源与限制等开发信息；复算事实和来源仍保留在内部结果与 `ALGORITHM_SOURCES.md`、`TraditionalAlgorithms/` 中。
...
- 2026-08-31 已更新未签名 `Divination.ipa`：Payload 为 iphoneos/arm64 `Divination.app`，版本 1.1 (1)，Bundle ID 为 `local.divination.app`、最低 iOS 16.0，包含本次手机六爻换行、局部选字、图标与算法资源；确认无 `_CodeSignature`、embedded mobileprovision、AppleDouble 或 `__MACOSX`，SHA-256 为 `ddc86c1277cbe6728594b45b144bc8a42f765690b3ef6aa95cd00394f841d7d5`。
...
当前决定版本 v2.0（由事件日期、ID 与标准类型推导）。 原生 App / IPA 的历史版本和验证保持原记录；文档整理未修改或重打产物。
