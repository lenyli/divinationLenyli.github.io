...
- 适配层、PWA/Apple 产物与 Windows 内嵌载荷版本均为 `mingyu-core-0.1.32+zhanbu-5`
- 构建输入：Mingyu Core 0.1.32 与 tyme4ts 1.5.2 的 MIT 源码；它们只在构建时使用，产品运行时只有已吸收的自包含静态包
- 运行方式：PWA 直接加载；iOS/macOS 通过系统 JavaScriptCore 加载同一文件；Windows 构建见下条更新流程
- 网络边界：运行时不调用 Mingyu API、Horosa、MCP 或任何在线服务
- 更新规则：算法包、适配层、golden/合同 fixture 和 `Divination-PWA/sw.js` 缓存版本必须同步更新；任一未同步时不得把源码修复描述成已完成的运行时验收
