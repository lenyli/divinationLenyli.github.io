...
function formatTarotInfo(data) {
    const cardLines = data.cards.map((card) => `- ${card.position}：${card.name}${card.reversed ? '（逆位）' : '（正位）'}${card.keywords.length ? `；关键词：${card.keywords.join('、')}` : ''}${card.element ? `；牌组属性：${card.element}` : ''}`);
    return [
        '占法：塔罗',
