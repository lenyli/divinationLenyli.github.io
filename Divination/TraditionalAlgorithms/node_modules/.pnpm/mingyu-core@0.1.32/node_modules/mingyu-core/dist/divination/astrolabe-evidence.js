...
function buildDistributionFacts(data, positionFacts) {
    ...
    return [
        ...Object.entries(data.summary.elements).map(([element, members]) => build(`distribution:element:${element}`, '元素', `${element}元素`, members, 'celestine 盘面元素归类')),
        ...Object.entries(data.summary.modalities).map(([modality, members]) => build(`distribution:modality:${modality}`, '模式', `${modality}模式`, members, 'celestine 盘面模式归类')),
        build('distribution:retrograde', '逆行', '逆行点', data.summary.retrograde, 'celestine 星体视运动状态汇总'),
        build('distribution:patterns', '盘面格局', '依赖库盘面格局', data.summary.patterns, 'celestine 盘面格局汇总'),
    ];
}
