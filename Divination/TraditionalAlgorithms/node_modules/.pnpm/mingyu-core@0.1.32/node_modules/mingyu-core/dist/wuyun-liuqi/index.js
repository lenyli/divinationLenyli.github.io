...
export function buildWuyunLiuqiPrompt(result, question, schools) {
    ...
    const sections = [
        ...
        [
            ...
            '五步主客运：',
            ...result.movementSteps.map((step) => `${step.order}. ${step.label}（${step.periodRule}）：主运${step.hostMovement.toneName}（${step.hostMovement.element}）；客运${step.guestMovement.toneName}（${step.guestMovement.element}）${step.guestRole ? `（${step.guestRole}）` : ''}；主客关系${step.hostGuestRelation.kind}`),
            '六步主客气：',
            ...result.qiSteps.map((step) => `${step.order}. ${step.label}（${step.solarTerms.join('、')}）：主气${step.hostQi.name}；客气${step.guestQi.name}${step.guestRole ? `（${step.guestRole}）` : ''}；主客关系${step.hostGuestRelation.kind}`),
        ].join('\n'),
    ];
