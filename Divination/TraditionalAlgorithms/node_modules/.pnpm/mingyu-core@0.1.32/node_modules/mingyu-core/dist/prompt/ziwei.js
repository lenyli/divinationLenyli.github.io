...
export function buildZiweiPromptDocument(options) {
    ...
    const user = joinPromptSections([
        ...
        buildPromptSection('任务', buildPromptTask(`请依据${scope === 'full' ? '本命与所列完整运限' : SCOPE_LABELS[scopes[0] ?? 'origin']}资料，${topicLabel ? `重点分析${topicLabel}，` : ''}先列出主要宫位、星曜、四化和运限证据，再回答问题。`, 'ziwei')),
    ]);
