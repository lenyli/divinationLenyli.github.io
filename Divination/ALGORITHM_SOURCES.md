...
| 方法 | 首版口径 | 主要实现依据 | 结果事实 |
|---|---|---|---|
| 六爻纳甲 | 手动六爻值或已冻结的六次三钱记录 | Mingyu Core `liuyao`；《京氏易传》《火珠林》《卜筮正宗》《增删卜易》 | 主变互卦、八宫世应、纳甲干支、六亲六神、旬空旺衰、月破日冲暗动、动变化进退、伏神、六合六冲与反伏吟事实 |
...
- 用神／类神层只做固定查表与盘面定位：六爻按事项映射六亲，并只在唯一明现或唯一伏藏时自动取用；大六壬直接使用上游天地盘的地盘支、天盘支、天将对应并保留类神候选；小六壬只保留主宫与原始口诀，不从文案关键词推断；梅花保持简化体用；太乙只映射辅助主客角色，不比较数值判胜负；金口诀输出四位六亲候选但不冒充唯一用神。事项定位随结构化结果输出，不调用 AI 二次判断。
- 所有共享算法显式记录 UTC 时间戳、计算墙钟、时区模式、偏移分钟和民用零点换日策略；四端当前产品口径固定为 UTC+08:00。
- 共享结果合同包含 `aiPromptVersion = zhanbu-ai-prompt-v1`、适合综合拼接的 `aiPromptSection` 与单项复制使用的 `aiPrompt`；问题文本只进入 Prompt，不反向参与排盘。
- 奇门首版固定时家转盘拆补；飞盘、置闰、日家/月家/年家未开放为 UI 选项。
- 金口诀未开放随机起课，避免不可复算的随机结果。
- 择日未输入参与人生辰时，不计算个人刑冲破害；UI、`displayedDays` 与 Prompt 只输出排序后的前 5 条，`allDays` 保留完整候选范围。
- 原始 `抽牌.xlsm` 的第 42/43 签截断已由 `source-data/xuantian-slip-42-43.json` 的限定补全源关闭；DATE12 水瓶三段日期重叠继续标记未决，不擅改区间。
...
- 当前适配层源码、PWA/Apple 静态包与 Windows 内嵌载荷均为 `mingyu-core-0.1.32+zhanbu-5`；生成物已刷新并通过 PWA/Windows golden 差分合同检查。
