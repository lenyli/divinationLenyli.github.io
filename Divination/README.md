...
除择日／黄历外，四端所有占卜方式都在问题输入框上方提供“所测何事”和“性别”；两项默认均为不选，婚恋／婚姻类必须选择性别，其他类别允许性别留空，寻人寻物类另提供寻找对象。支持事项定位的术数会固定输出对应的用神／类神：六爻含纳甲六亲与伏神，大六壬含十二天将完整分布，小六壬含分类口诀摘句，太乙含主客算语义，金口诀含四位六亲；梅花继续采用简化体用口径。

PWA 在手机宽度下采用与 iOS 相同的四列四行导航：首页／择日／空位／中英切换位于第一行，其余模块按指定顺序排列；桌面宽度仍使用两排导航。
...
综合占卜的复制文本按“问题背景—卡牌与卦象—传统术数合参—AI 解读要求”分段，明确列出每种方法，不再连续拼接；事项分类会传给六爻、大六壬、小六壬、梅花、太乙和金口诀。择日／黄历提示及灵签正文不进入这份 AI 输出。
