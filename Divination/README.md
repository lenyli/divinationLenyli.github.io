...
| 六爻纳甲 | `TRI_ELEM`、`HEXAGRAMS`、`POS` + 共享适配层源码 | 64 卦；共享层已补纳甲干支、世应、六亲六神、旺衰冲破暗动、伏神与动变事实，等待重新生成四端算法资源后替换旧运行路径 |
...
| 玄天灵签 | `QIAN` | 49 签，每签 12 个字段；第 42/43 签在原始 `抽牌.xlsm` 已截断，运行时明确标记资料不完整且不猜补 |
...
各端共有：子标签、页面状态缓存、每模块 30 条历史记录、结果一键复制（格式为「问题：占卜结果」，便于粘给 AI 解读）。PWA 新记录同时保存结构化记录对象，并兼容读取旧版纯文本历史；Apple/Windows 的卡牌历史仍是旧文本格式，后续需迁移。
...
本轮按 `/Users/liminghan/Downloads/divination-module-guides` 完成源码修复：统一固定东八区计算上下文；卡牌／符文实体级公平抽取；首页同一时刻冻结与失败显式化；太乙、六壬、小六壬、梅花、金口诀、择日的事实边界收紧；六爻完整共享结果源码；PWA 单一 Service Worker 注册和缓存版本更新。
